...
const sketch = props => {
  ...
  function vecField(x, y) {
    ...
    return [
      Math.max(y, Math.log(Math.abs(Math.sqrt(x * x + y * y)))) - y,
      Math.exp(x),
    ];
    return [y, x];
    return [y, Math.sqrt(x * x + y * y)];
    return [x - y, y];
    return [Math.sqrt(x * x + y * y), Math.cos(x)];
    return [
      Math.cos(
        Math.sin(
          Math.max(x - (x - Math.max(x, Math.sqrt(x * x + y * y))), x * y),
        ),
      ),
      Math.sin(y / y),
    ];
    return [Math.sin(y) - Math.cos(y), Math.sqrt(x * x + y * y)];
    return [x / Math.abs(Math.sqrt(x * x + y * y)), -Math.sqrt(x * x + y * y)];
    return [y, Math.sqrt(x * x + y * y) / x];
    return [x - y, Math.sqrt(x * x + y * y) / x];
  ...
  for (let y = 0; y < gridSize; y++) {
    ...
    for (let x = 0; x < gridSize; x++) {
      ...
      // lines.push([
      //   [tx - size, ty + size * Math.sin(heading)],
      //   [tx + size, ty + size * Math.sin(heading)],
      // ]);

      // line.push([tx + size * Math.cos(heading), ty + size * Math.sin(heading)]);
    }
    // lines.push(line);
  }
